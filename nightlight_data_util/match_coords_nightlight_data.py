...
def match_coords_w_nightlight_data(
    ...
    for light_variable, url in raster_urls.items():
        ...
        zoneStats = QgsZonalStatistics(
            vlayer, raster, stats=QgsZonalStatistics.Statistics(QgsZonalStatistics.Mean))
    ...
    QgsVectorFileWriter.writeAsVectorFormatV2(
        vlayer, destUrl, transformContext, options)
    ...
    for light_variable in raster_urls.keys():
        if count == 0:
            light_df.rename(
                columns={"mean": light_variable + "_aggregated_mean"}, inplace=True)
        else:
            light_df.rename(
                columns={"mean_" + str(count): light_variable + "_aggregated_mean"}, inplace=True)
